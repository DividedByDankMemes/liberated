...
execute if score $in.uid rx.pdb.io >= $uid.next rx.uid run tellraw @a[tag=rx.admin] ["", {"nbt": "playerdb.name", "storage": "rx:info"}, {"text": " "}, {"text": "Get Error: Input uid above max uid", "color": "#CE4257"}]
