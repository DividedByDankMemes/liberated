...
execute if score @s rx.uuid0 = @s rx.uuid0 if score @s rx.pdb.hasEntry matches 0.. run scoreboard players operation @s rx.pdb.has_entry = @s rx.pdb.hasEntry
...
execute if score @s rx.uuid0 = @s rx.uuid0 store result storage rx.playerdb:main uuid[{selected: 1b}].entries[-1].has_entry byte 1 run scoreboard players get @s rx.pdb.has_entry
