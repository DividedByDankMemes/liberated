...
execute if score $verified rx.pdb.io matches 0 unless data storage rx.playerdb:main players[] run tellraw @a[tag=rx.admin] ["", {"nbt": "playerdb.name", "storage": "rx:info"}, {"text": " "}, {"text": "Select Error: Selection failed. No players in database to select", "color": "#CE4257"}]
